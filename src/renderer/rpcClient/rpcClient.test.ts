...
describe("rpcClient", () => {
  ...
  it.only("returns results for an RPC request", async () => {
  ...
  it.only("returns the results from a cache if a repeated request is made within a cache period", async () => {
    ipcService.rpcRequest.mockImplementation(() => ({
  ...
  it.only("returns fresh results if cache expired", async () => {
    ipcService.rpcRequest.mockImplementation(() => ({
  ...
  it.only("throws an error if error is not fixable", async () => {
    ipcService.rpcRequest.mockImplementation(() => ({
      result: null,
      method: "getblockhash",
      // @ts-ignore
