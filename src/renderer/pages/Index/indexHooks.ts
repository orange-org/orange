...
export const useRpcServerStatus = () => {
  ...
  usePolling(async () => {
    try {
      // Requesting uptime is a lightweight call to help us probe
      // the current status of the RPC server.
      //
      // On start, we're interested in checking if the server is
      // still warming up. On shutdown, we're interested to see
      // if the server has shutdown.
